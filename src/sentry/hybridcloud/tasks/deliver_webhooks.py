...
def _schedule_next_attempts_bulk(failed_records: list[WebhookPayload]) -> None:
    ...
    and backoff schedule times in a single UPDATE query per attempt-group.

    Records are grouped by their current attempt count so that each group gets
    the correct backoff duration. This replaces N individual UPDATE queries with
    at most ``MAX_ATTEMPTS`` queries (one per distinct attempt value).
    """
    if not failed_records:
        return

    # Group records by their current attempts so we can compute the correct
    # backoff for each group and issue one UPDATE per group.
    by_attempts: dict[int, list[int]] = {}
    for record in failed_records:
        attempts = record.attempts
        by_attempts.setdefault(attempts, []).append(record.pk)
    ...
    for attempts, ids in by_attempts.items():
        ...
        WebhookPayload.objects.filter(id__in=ids).update(
            ...
            schedule_for=new_schedule_for,
        )

    # Update in-memory state so callers see consistent values.
    for record in failed_records:
    ...
    try:
        while True:
            ...
            for record in query[:100]:
                ...
                # inner loop (up to 30s timeout x 100 records) cannot outlast the
                ...
                try:
                    ...
                    # continue. It has been collected for bulk retry scheduling.
                    continue
                except Exception:
                    # Unexpected error (e.g. ValueError). Collect the record for
                    # bulk retry scheduling before letting the exception propagate.
                    failed_records.append(record)
                    _schedule_next_attempts_bulk(failed_records)
                    raise

            # Bulk-update all failed records from this batch slice.
            _schedule_next_attempts_bulk(failed_records)
...
def deliver_message(payload: WebhookPayload) -> None:
    ...
    On success the payload is deleted. On failure (DeliveryFailed or unexpected
    exception) the exception propagates to the caller which is responsible for
    bulk-scheduling retries via ``_schedule_next_attempts_bulk``.
    """
