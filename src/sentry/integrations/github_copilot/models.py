...
class GithubCopilotTaskResponse(BaseModel):
    ...
    def normalize_response(cls, values: dict) -> dict:
        # If 'task' key exists and its value is a dict, treat as wrapped format
        if "task" in values and isinstance(values["task"], dict):
            return values
        # If 'task' key is missing but 'id' exists at top level, treat as unwrapped format
        if "task" not in values and "id" in values:
            return {"task": values}
        return values
