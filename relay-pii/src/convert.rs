...
/// Matches `'http.request.header.cookie'` as well as any entry thereof.
///
...
-----END RSA PRIVATE KEY-----"""#;
    ...
    fn test_span_v2_cookie() {
        let mut data = SpanV2::from_value(
            serde_json::json!({
              "attributes": {
                "http.request.header.cookie": {"value": ["session=foobar", "language=en"]},
