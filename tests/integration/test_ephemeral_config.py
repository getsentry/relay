...
def test_store_via_ephemeral_relay(mini_sentry, get_relay_binary, background_process, random_port, mode, config_method):
    project_id = 42
    version = "latest"
    ...
    relay_id = ""
    if mode == "managed":
        credentials = json.loads(subprocess.check_output(relay_bin + ["credentials", "generate", "--stdout"], universal_newlines=True))
    ...
    if config_method == "arg":
        args += ["--no-processing", "--upstream", upstream, "--port", str(port), "--mode", mode]
        if mode == "managed":
            args += ["--id", relay_id, "--public-key", public_key, "--secret-key", secret_key]
    ...
    process = background_process(args, env=env)

    relay = Relay(("127.0.0.1", port), process, mini_sentry, public_key, secret_key, relay_id, None, {}, version)
