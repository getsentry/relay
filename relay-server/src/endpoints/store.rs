...
fn parse_envelope(meta: RequestMeta, data: Bytes) -> Result<Envelope, BadStoreRequest> {
    ...
    // Compared to raw event JSON, we assume that clients send more sanitized event payloads.
    // However, clients do not have to send an event_type, which is required by Relay to
    // emit proper outcomes and enforce rate limits.
    if let Some(event) = envelope.items_mut().find(|i| i.ty() == ItemType::Event) {
        if event.event_type().is_none() {
            let minimal = common::minimal_event_from_json(&event.payload())?;
            event.set_event_type(minimal.ty);
        }
    }
