//! Endpoint for Cross-Origin-Opener-Policy (COOP) reports.
//!
...
struct CoopReportParams {
    meta: RequestMeta,
    body: Bytes,
}

fn is_coop_mime(mime: Mime) -> bool {
...
/// Handles all messages coming on the COOP endpoint.
async fn handle(
    state: ServiceState,
    mime: Mime,
    params: CoopReportParams,
) -> axum::response::Result<impl IntoResponse> {
    if !is_coop_mime(mime) {
    ...
    for item in items {
        let mut report_item = Item::new(ItemType::Coop);
