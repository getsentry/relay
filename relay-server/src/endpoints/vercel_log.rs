...
use crate::integrations::{Integration, LogsIntegration, VercelFormat};
...
async fn handle(
    ...
    let format = match ContentType::from(content_type.as_ref()) {
        ContentType::Json => VercelFormat::Json,
        _ => return Ok(StatusCode::UNSUPPORTED_MEDIA_TYPE),
    };
    let integration = Integration::Logs(LogsIntegration::VercelV1 { format });
