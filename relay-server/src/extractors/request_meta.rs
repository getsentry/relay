...
pub enum BadEventMeta {
    ...
    #[error("bad x-sentry-relay-signature header")]
...
impl IntoResponse for BadEventMeta {
    fn into_response(self) -> Response {
        let code = match self {
            ...
            | Self::BadProject(_)
            | Self::BadPublicKey(_)
            | Self::SignatureError(_) => StatusCode::BAD_REQUEST,
        };
...
impl RequestTrust {
    /// Returns `true` if self is [`Self::Trusted`].
    pub fn is_trusted(self) -> bool {
        matches!(self, Self::Trusted)
    }

    /// Returns `true` if self is [`Self::Untrusted`].
...
impl PartialMeta {
    ...
    pub fn copy_to(self, mut complete: RequestMeta) -> RequestMeta {
        ...
        if self.request_trust.is_some() {
            complete.request_trust = self.request_trust;
        }
...
impl FromRequestParts<ServiceState> for PartialMeta {
    ...
    async fn from_request_parts(
        ...
        let request_trust = Some(match from_internal_relay {
...
mod tests {
    ...
    impl RequestMeta {
        ...
        pub fn new(dsn: relay_common::Dsn) -> Self {
            Self {
                ...
                user_agent: Some("sentry/agent".to_owned()),
                no_cache: false,
                received_at: Utc::now(),
                client_hints: ClientHints::default(),
                request_trust: None,
    ...
    fn test_request_meta_roundtrip() {
        ...
        let reqmeta = RequestMeta {
            ...
            forwarded_for: "8.8.8.8".to_owned(),
            user_agent: Some("0x8000".to_owned()),
            no_cache: false,
            received_at: Utc::now(),
    ...
    fn test_signature_not_serialized() {
        ...
        with_signature.signature = Some(Signature("test-signature".to_owned()));
    ...
    #[test]
    fn test_trusted_is_not_untrusted() {
        let x = RequestTrust::Trusted;
        assert!(x.is_trusted());
        assert!(!x.is_untrusted());

        let x = RequestTrust::Untrusted;
        assert!(!x.is_trusted());
        assert!(x.is_untrusted());
    }
}
