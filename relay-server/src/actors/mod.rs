...
//!  - [`EventManager`](envelopes::EventManager) and [`EventProcessor`](envelopes::EventProcessor):
//!    Handle a queue of envelopes, verify their projects, execute PII stripping and finally send
//!    the envelope to the upstream. The processor is spawned in multiple synchronous worker threads
//!    (via `SyncArbiter`).
