use relay_event_normalization::coop;
use relay_event_schema::protocol::{CoopReportError, OurLog};
...
pub fn convert_to_logs(envelope: &mut ManagedEnvelope) {
    let items = envelope
        .envelope_mut()
        .take_items_by(|item| item.ty() == &ItemType::Coop);
    let mut logs = ContainerItems::new();

    for item in items {
        match log_from_coop_item(&item, envelope.meta()) {
...
fn log_from_coop_item(
    ...
    let report = Annotated::from_json_bytes(&payload)
        .map_err(CoopReportError::InvalidJson)
        .map_err(ProcessingError::InvalidCoopReport)?;
    let Some(log) = coop::create_log(report, meta.received_at()) else {
        return Ok(None);
    };
