...
mod tests {
    ...
    // Checks that the trace rules matches on the parametrized name of the root ds ctransaction.
    #[test]
    fn test_trace_matches_parametrized_dsc_root_transaction() {
        let dsc = DynamicSamplingContext {
            trace_id: Uuid::new_v4(),
            public_key: ProjectKey::parse("abd0f232775f45feab79864e580d160b").unwrap(),
            release: Some("1.1.1".to_string()),
        ...
        // Get a sampling config that matches a specific transaction name.
        let get_config = |condition: &str| -> SamplingConfig {
            let param_condition = EqCondition::new("trace.transaction", condition);
            let sampling_rule = SamplingRule {
                condition: RuleCondition::Eq(param_condition),
                sampling_value: SamplingValue::SampleRate { value: 1.0 },
                ty: RuleType::Trace,
                id: RuleId(0),
                time_range: TimeRange::default(),
            ...
            SamplingConfig {
                rules: vec![sampling_rule],
                ..SamplingConfig::new()
            }
        };

        // checks if a dsc matches a specific sampling config.
        let is_match = |dsc, config| {
        ...
        // This matches the transaction name in the dsc.
        let unparam_config = get_config("/auth/login/test/");
        assert!(is_match(&dsc, &unparam_config));

        // This parametrized version does not match because we didn't run parametrization.
        let param_config = get_config("/auth/login/*/");
        assert!(!is_match(&dsc, &param_config));
        ...
        let dsc = envelope.dsc().unwrap();

        // this time, the parametrized name matches.
        assert!(is_match(dsc, &param_config));

        // and the unparametrized does not match.
        assert!(!is_match(dsc, &unparam_config));
    }
}
