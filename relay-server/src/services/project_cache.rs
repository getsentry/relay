...
use crate::services::metrics_aggregator::{Aggregator, FlushBuckets};
...
impl ProjectCacheBroker {
    ...
    fn handle_flush_buckets(&mut self, message: FlushBuckets) {
        let buckets = message
            ...
            .into_iter()
            .filter_map(|buckets| {
                let project = self.get_or_create_project(buckets.project_key());
                project.check_buckets(buckets)
            })
            .collect();

        self.services
            .envelope_processor
            .send(EncodeMetrics { buckets })
    }
