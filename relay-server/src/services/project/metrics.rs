...
pub fn filter_namespace(bucket: &Bucket, source: BucketSource) -> ItemAction {
    match bucket.name.namespace() {
        MetricNamespace::Sessions => ItemAction::Keep,
        MetricNamespace::Transactions => ItemAction::Keep,
        MetricNamespace::Spans => ItemAction::Keep,
        MetricNamespace::Profiles => ItemAction::Keep,
        MetricNamespace::Custom => ItemAction::Keep,
        MetricNamespace::Stats if source == BucketSource::Internal => ItemAction::Keep,
        MetricNamespace::Stats => ItemAction::DropSilently,
        MetricNamespace::Unsupported => ItemAction::DropSilently,
    }
}

pub fn apply_project_state(bucket: &mut Bucket, project_state: &ProjectState) -> ItemAction {
    if !is_metric_namespace_valid(project_state, bucket.name.namespace()) {
        ...
        return ItemAction::Drop(Outcome::Filtered(FilterStatKey::DisabledNamespace));
    };
    ...
    if let ErrorBoundary::Ok(ref metric_config) = project_state.config.metrics {
        if metric_config.denied_names.is_match(&*bucket.name) {
            ...
            return ItemAction::Drop(Outcome::Filtered(FilterStatKey::DeniedName));
        }
