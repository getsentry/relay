...
use relay_general::protocol::EventType;
use relay_quotas::{
...
/// Map an event type to the corresponding data category.
pub fn data_category_from_event(event_type: EventType) -> DataCategory {
    match event_type {
        EventType::Default => DataCategory::Default,
        EventType::Error => DataCategory::Error,
...
fn infer_event_category(item: &Item) -> Option<DataCategory> {
    match item.ty() {
        ItemType::Event => item.event_type().map(data_category_from_event),
        ItemType::SecurityReport => Some(DataCategory::Security),
