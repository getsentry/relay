    ...
    def __init__(
        ...
        Args:
            ...
            organization_id: Optional. The organization ID to look up the org_integration.
                           If provided (and org_integration_id is not), will find the
                           correct org_integration for this organization.

        Note: Providing org_integration_id or organization_id ensures the correct
        organization's credentials are used when proxying requests through control silo.
        If neither is provided, falls back to infer_org_integration() which may return
        the wrong organization's credentials in multi-org scenarios.
        """
        self.integration = integration
        self.metadata = self.integration.metadata
        self.base_url = self.metadata["service_url"].rstrip("/")

        # Determine the correct org_integration_id to use
        ...
        # This maintains backward compatibility but may use wrong credentials
