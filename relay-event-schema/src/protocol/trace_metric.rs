...
pub struct TraceMetric {
    ...
    /// The metric name.
    #[metastructure(required = true, trim = false)]
    pub metric_name: Annotated<String>,

    /// The metric type.
    #[metastructure(required = true)]
    pub metric_type: Annotated<MetricType>,
...
impl Getter for TraceMetric {
    fn get_value(&self, path: &str) -> Option<relay_protocol::Val<'_>> {
        Some(match path.strip_prefix("metric.")? {
            "metric_name" => self.metric_name.as_str()?.into(),
            "value" => relay_protocol::Val::F64(*self.value.value()?),
            path => {
...
mod tests {
    ...
    #[test]
    fn test_metric_type_validation() {
        let json = r#"{
            "timestamp": 1544719860.0,
            "trace_id": "5b8efff798038103d269b633813fc60c",
