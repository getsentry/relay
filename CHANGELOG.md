...
- Compare releases by version in the `gt`, `gte`, `lt`, and `lte` conditions of generic inbound filters. Release fields such as `event.release` and the `sentry.release` attribute order like Sentry's release search instead of as plain strings. ([#6430](https://github.com/getsentry/relay/pull/6430))
