...
- Rename `relay.event.accepted`, `relay.event.rejected` and `relay.event.processing_time` to `relay.envelope.*` respectively as they no longer measure events exclusively. **This is a breaking change.** The former two metrics also have a new bool `is_event` tag.
