...
impl AggregatorRouter {
        ...
        Self {
            ...
            secondary_aggregators: secondary_aggregators
                ...
                .map(|c| {
                    ...
                    (namespace, aggregator::Aggregator::named(c.name, c.config))
                })
                .collect(),
        }
    }
    ...
    fn aggregators(&self) -> impl Iterator<Item = &aggregator::Aggregator> {
    ...
    pub fn merge_buckets(
        ...
        for (namespace, group) in metrics_by_namespace.into_iter() {
            ...
            aggregator.merge_all(project_key, group, max_total_bucket_bytes);
        }
    }
