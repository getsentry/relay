...
struct BucketKey {
    ...
    metric_name: MetricResourceIdentifier<'static>,
...
struct BucketKeyRef<'a> {
    project_key: ProjectKey,
    timestamp: UnixTimestamp,
    metric_name: &'a MetricResourceIdentifier<'static>,
...
impl Aggregator {
    ...
    fn validate_metric_name(
        ...
        if metric_name_length > aggregator_config.max_name_length {
            ...
            return Err(AggregateMetricsErrorKind::InvalidStringLength(
    ...
    fn normalize_metric_name(key: &mut BucketKey) -> Result<(), AggregateMetricsError> {
        if matches!(key.metric_name.namespace, MetricNamespace::Unsupported) {
...
mod tests {
    ...
    fn test_aggregator_cost_tracking() {
        ...
        let bucket_key = BucketKey {
            project_key,
            timestamp: UnixTimestamp::now(),
            metric_name: "c:transactions/foo@none".parse().unwrap(),
            tags: BTreeMap::new(),
        };
    ...
    fn test_validate_bucket_key_chars() {
        ...
        let bucket_key = BucketKey {
            project_key,
            timestamp: UnixTimestamp::now(),
            metric_name: "c:transactions/hergus.bergus".parse().unwrap(),
            tags: {
        ...
        bucket_key.metric_name = "hergus\0bergus".parse().unwrap();
    ...
    fn test_validate_bucket_key_str_lens() {
        ...
        let short_metric = BucketKey {
            project_key,
            timestamp: UnixTimestamp::now(),
            metric_name: "c:transactions/a_short_metric".parse().unwrap(),
            tags: BTreeMap::new(),
        };
        ...
        let short_metric_long_tag_key = BucketKey {
            project_key,
            timestamp: UnixTimestamp::now(),
            metric_name: "c:transactions/a_short_metric_with_long_tag_key".parse().unwrap(),
        ...
        let short_metric_long_tag_value = BucketKey {
            project_key,
            timestamp: UnixTimestamp::now(),
            metric_name: "c:transactions/a_short_metric_with_long_tag_value".parse().unwrap(),
