...
fn sql_tables_from_query(db_system: &str, query: &str) -> Option<String> {
...
mod tests {
    ...
    fn extract_table_select() {
        ...
        assert_eq!(sql_tables_from_query("postgresql", query).unwrap(), ",b,");
    }

    #[test]
    fn extract_table_select_nested() {
        let query = r#"SELECT * FROM (SELECT * FROM "a.b") s WHERE "x" = 1"#;
        assert_eq!(sql_tables_from_query("postgres", query).unwrap(), ",b,");
    }
    ...
    fn extract_table_multiple() {
        ...
        assert_eq!(
            sql_tables_from_query("postgresql", query).unwrap(),
            ",a,b,c,"
        );
    }
    ...
    fn extract_table_multiple_mysql() {
        ...
        assert_eq!(sql_tables_from_query("mysql", query).unwrap(), ",a,b,c,");
    }
...
LIMIT 1
    ...
    fn extract_table_delete() {
        ...
        assert_eq!(sql_tables_from_query("sqlite", query).unwrap(), ",b,");
    }

    #[test]
    fn extract_table_insert() {
        let query = r#"INSERT INTO "a" ("x", "y") VALUES (%s, %s)"#;
        assert_eq!(sql_tables_from_query("postgresql", query).unwrap(), ",a,");
    }
