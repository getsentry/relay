...
pub struct ScopedAggregatorConfig {
    ...
    pub condition: AggregatorCondition,
...
pub enum AggregatorCondition {
    ...
    Not {
        ...
        inner: Box<AggregatorCondition>,
    },
}

impl AggregatorCondition {
    /// Checks if the condition matches the given namespace.
    pub fn matches(&self, namespace: Option<MetricNamespace>) -> bool {
        match self {
            AggregatorCondition::Eq(field) => field.matches(namespace),
            AggregatorCondition::And { inner } => inner.iter().all(|cond| cond.matches(namespace)),
            AggregatorCondition::Or { inner } => inner.iter().any(|cond| cond.matches(namespace)),
            AggregatorCondition::Not { inner } => !inner.matches(namespace),
        }
    }
}

/// Defines a field and a field value to compare to when a [`AggregatorCondition`] is evaluated.
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(tag = "field", content = "value", rename_all = "lowercase")]
pub enum AggregatorFieldCondition {
    /// Field that allows comparison to a metric or bucket's namespace.
    Namespace(MetricNamespace),
}

impl AggregatorFieldCondition {
    fn matches(&self, namespace: Option<MetricNamespace>) -> bool {
        match (self, namespace) {
            (AggregatorFieldCondition::Namespace(expected), Some(actual)) => expected == &actual,
            _ => false,
        }
    }
}
