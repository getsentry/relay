...
pub fn should_filter(event: &Event, config: &ReleasesFilterConfig) -> Result<(), String> {
    let filtered_releases = &config.releases;
    ...
    fn test_release_filtering() {
        ...
        for &(release, blocked_releases, expected) in examples {
            ...
            let config = ReleasesFilterConfig {
                releases: blocked_releases.iter().map(|r| r.to_string()).collect(),
            };
