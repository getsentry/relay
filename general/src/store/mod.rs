...
impl<'a> Processor for StoreNormalizeProcessor<'a> {
    ...
    fn process_event(&mut self, event: &mut Annotated<Event>, state: ProcessingState) {
        schema::SchemaProcessor.process_event(event, state.clone());
        ...
        escalate::EscalateErrorsProcessor.process_event(event, state.clone());

        // Do this before trimming such that we do not needlessly trim other if we're just going to
        // throw it away
        remove_other::RemoveOtherProcessor.process_event(event, state.clone());

        // Trimming should happen at end to ensure derived tags are the right length.
        self.trimming_processor.process_event(event, state);
    }

    fn process_breadcrumb(
        &mut self,
        breadcrumb: &mut Annotated<Breadcrumb>,
    ...
    fn process_client_sdk_info(
        &mut self,
        info: &mut Annotated<ClientSdkInfo>,
        state: ProcessingState,
    ) {
        ProcessValue::process_child_values(info, self, state);
        if info.0.is_none() {
            (*info).0 = self.config.get_sdk_info();
        }
    }

    fn process_exception(&mut self, exception: &mut Annotated<Exception>, state: ProcessingState) {
    ...
    fn process_frame(&mut self, frame: &mut Annotated<Frame>, state: ProcessingState) {
    ...
    fn process_stacktrace(
        ...
        if let Some(limit) = self.config.stacktrace_frames_hard_limit {
            stacktrace::enforce_frame_hard_limit(stacktrace, limit);
        }
